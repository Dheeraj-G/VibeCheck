...
function extractApiFromBaseUrl(openapi) {
    if (openapi.servers && openapi.servers.length > 0) {
        return openapi.servers[0].url;
    }
    return '';
}
function getAuthHeaders(openapi, operation) {
    const headers = {};
...
export function createMCPServer({ name = 'spiceflow', version = '1.0.0', openapi, fetch = defaultFetch, paths, ignorePaths, baseUrl = '', }) {
    ...
    async function fetchWithBaseServerAndAuth(u, options, operation, userHeaders, userCookies) {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
        ...
        const tools = filteredPaths.flatMap(([path, pathObj]) => Object.entries(pathObj || {})
            ...
            const { queryParams, pathParams, headerParams, cookieParams } = getOperationParameters(operation);
            if (queryParams) {
            ...
            let moreDescription = operation.description ||
                operation.summary;
            if (moreDescription) {
                description += '. ';
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            const userCookies = args.cookies;
            ...
            const response = await fetchWithBaseServerAndAuth(path, {
                method,
                headers: {
                ...
                body: body
                    ? typeof body === 'string'
                        ...
                        : JSON.stringify(body)
                    : undefined,
            }, operation, userHeaders, userCookies);
    ...
    server.setRequestHandler(ListResourcesRequestSchema, async () => {
        ...
        for (const [path, pathObj] of Object.entries(openapi.paths)) {
            ...
            if (getOperation && !path.includes('{')) {
                const { queryParams, headerParams, cookieParams } = getOperationParameters(getOperation);
                const hasRequiredQuery = queryParams?.required && queryParams.required.length > 0;
                const hasRequiredHeaders = headerParams?.required && headerParams.required.length > 0;
                const hasRequiredCookies = cookieParams?.required && cookieParams.required.length > 0;
...
function formatToolName(nameToFormat, method, pathForMap) {
    ...
    if (!regex.test(formatted)) {
        throw new Error(`Formatted tool name "${formatted}" (from original: '${nameToFormat}') does not match required pattern: ^[a-zA-Z0-9_-]{1,64}$`);
    }
    ...
    if (existingEntry &&
        ...
        console.error(new Error(`Duplicate tool name generated: '${formatted}'. ` +
